...
extension String {
    public static func dateStrFromInt(intTime: NSTimeInterval) -> String {
        let date = NSDate(timeIntervalSince1970: intTime / 1000)
        
        let dfmt = NSDateFormatter()
        
        let now = NSDate()
        let time = NSInteger(now.timeIntervalSinceDate(date))
    ...
    public static func base64StringFromImage(image: UIImage) -> String{
        /*
    ...
    public static func urlString(str: String) -> String{
        let urlStr = ""
        return urlStr
    }
    public static func sizeFormate(size:Float) -> String {
        if size == 0 {
            return ""
        }else if size < 1024 {
            return "\(size)B"
    ...
    public func isEmail() -> Bool {
        ...
        do {
            ...
            let matches = regex.matchesInString(self, options: NSMatchingOptions.ReportProgress, range: NSMakeRange(0, self.characters.count))
